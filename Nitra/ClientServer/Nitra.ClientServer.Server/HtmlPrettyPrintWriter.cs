...
namespace Nitra.PrettyPrint
{
  internal class HtmlPrettyPrintWriter : PrettyPrintWriter
  {
    ...
    private IEnumerable<SpanClass> _spanClasses;

    public HtmlPrettyPrintWriter(PrettyPrintOptions options, IEnumerable<SpanClass> spanClasses, string missingNodeClass, string debugClass, string garbageClass)
      : base(options)
    {
    ...
    protected override void FormatString(IPrettyPrintSource _source, NSpan token, string text, SpanClass spanClass)
    {
      if (token.IsEmpty)
      {
    ...
    private string MakeHtml(string html)
    {
      var spanStyles = new StringBuilder();
      foreach (var spanClass in _spanClasses)
      {
        var rgb = spanClass.Style.ForegroundColor;
        var r   = (byte)((rgb & 0xFF0000) >> 16);
        var g   = (byte)((rgb & 0x00FF00) >> 8);
        var b   = (byte) (rgb & 0x0000FF);
        spanStyles.Append('.').Append(spanClass.FullName.Replace('.', '-'))
          .Append("{color:rgb(").Append(r).Append(',').Append(g).Append(',').Append(b).AppendLine(");}");
      }

      return Resources.PrettyPrintDoughnut.Replace("{spanclasses}", spanStyles.ToString()).Replace("{prettyprint}", html);
    }
    ...
    public override string ToString()
    {
      ...
      {
        WriteTo(result);
        return MakeHtml(result.ToString());
      }
    }
  }
}
